...
public class BookingController implements IBookingController {
	...
	enum BookingState {
		SELECT_MOVIE, FILTER_CINEPLEX, SELECT_CINEPLEX, SELECT_SHOWTIME, SEAT_BOOKING, CONFIRMATION, FINISH
	}

	public void doBooking(Customer customer, Movie movie) {
		// Select movie -> filter by movie -> return cineplexes -> chooses cineplex ->
		// filter by movie and cineplex-> return showtime
		...
		while (!done)
			switch (state) {
				...
				case SELECT_SHOWTIME:
					List<Showtime> showtimeList = MOBLIMA.showtimeController.filterShowtimeByMovieAndCineplex(movie,
							cineplex);
	...
	// cineplexList = MOBLIMA.showtimeController.filterCineplexByMovie(movie);
	// if (cineplexList.size() == 0) {
	// System.out.println("There are no cineplexes showing this movie. Going
	// back...");
	// state = BookingState.SELECT_MOVIE;
	// break;
	// }
	...
	// MOBLIMA.showtimeController.filterShowtimeByMovieAndCineplex(movie, cineplex);
	// showtime = MOBLIMA.showtimeController.selectShowtime(showtimeList);
